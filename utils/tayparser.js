  ...
  parse(MarkdownText) {
    let HtmlCode = this.escape(MarkdownText, this["data"].escapeHTML || false)
      ...
      .replace(
        /^\s*\n\*/gm,
        `<ul style="${
          this["data"].ul.extendStyle || " "
        }" class="list-disc ml-4 ${this["data"].ul.customClass || " "}">\n*`
      )
      .replace(/^(\*.+)\s*\n([^\*])/gm, `$1\n</ul>\n\n$2`)
      .replace(
        /^\*(.+)/gm,
      ...
      .replace(
        ...
        }" style='${this["data"].del.extendStyle || " "}'>$1</del>`
      )
      .replace(
        /[\-]{2}([^\-]+)[\-]{2}/g,
        `<del style='${
          this["data"].del.extendStyle || " "
        }' class="line-through ${this["data"].del.customClass || " "}">$1</del>`
      )
      .replace(
        /[\_]{2}([^\_]+)[\_]{2}/g,
